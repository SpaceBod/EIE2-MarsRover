    ...
    if(WiFi.status()== WL_CONNECTED){
      ...
      /*   JSONVar myObject = JSON.parse(JSON.stringify(payload));
       ...
       JSONVar keys = myObject.keys();
        ...
        //__________________manual control____________________________________________
            ...
            if (int(md.x)<=300)
              {
                ...
                {
                  real_x = sqrt(real_x*real_x);
                  Serial.println("Left motor Speed: "+ String(50+10*(real_x*real_x)));
                  Serial.println("Right motor Speed: "+ String(50+10*(-real_x*real_x)));
                  robot.rotate(left_motor, 50+10*(+real_x*real_x), direction);
                  robot.rotate(right_motor, 50+10*(-real_x*real_x), direction);//run right_motor at 60% speed in CW direction
                }
                if (real_x<0)
                {
                  real_x = sqrt(real_x*real_x);
                  Serial.println("Right motor Speed: "+ String(50+10*(+real_x*real_x)));
                  Serial.println("Left motor Speed: "+ String(50+10*(-real_x*real_x)));
                  robot.rotate(right_motor, 50+10*(+real_x*real_x), direction);
                  robot.rotate(left_motor, 50+10*(-real_x*real_x), direction);//run right_motor at 60% speed in CW direction
                }
                if(real_x == 0){
                  robot.rotate(right_motor,50, direction);//run right_motor at 60% speed in CW direction
                  robot.rotate(left_motor, 50, direction);//run right_motor at 60% speed in CW direction
                }
              }
              else{
