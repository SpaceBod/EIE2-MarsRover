  ...
  */
  ...
  if(md.y < 1000)
  {
    if (real_x>0)
    {
      ...
      Serial.println("Left motor Speed: "+ String(78+10*(+real_x*real_x)));
      Serial.println("Right motor Speed: "+ String(70+10*(-real_x*real_x)));
      robot.rotate(left_motor, 80+10*(+real_x*real_x), CW);
      robot.rotate(right_motor, 80+10*(-real_x*real_x), CW);//run right_motor at 60% speed in CW direction
    }
    if (real_x<0)
    {
      real_x = sqrt(real_x*real_x);
      Serial.println("Right motor Speed: "+ String(80+10*(+real_x*real_x)));
      Serial.println("Left motor Speed: "+ String(70+10*(-real_x*real_x)));
      robot.rotate(right_motor, 80+10*(+real_x*real_x), CW);
      robot.rotate(left_motor, 80+10*(-real_x*real_x), CW);//run right_motor at 60% speed in CW direction
    }
    if(real_x == 0){
